...
public class AccountsValidationRabbitMQAdapter implements AccountsValidationSPI {
    ...
    public boolean hasOpenAccounts(Long clientId) {
        ...
        AccountCheckResponseMessage response = (AccountCheckResponseMessage) rabbitTemplate.convertSendAndReceive(
        ...
        if (response == null) {
            log.warn("No response from accounts service for clientId={}. Allowing deactivation.", clientId);
